@component('admin.layouts.content',['title'=>' ویرایش کد تخفیف  '])
    ...
    @slot('breadcromp')
        ...
        <li class="breadcrumb-item"><a>ویرایش کد تخفیف </a></li>
    @endslot
    @slot('script')
        <script>
            $('#users').select2({
    ...
    <div class="card card-info">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form action="{{route('admin.discounts.update',$discount->id)}}" method="post" class="form-horizontal">
            @csrf
            @method('PATCH')
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control" name="code" id="inputEmail3"
                               placeholder="عنوان کد تخفیف را اینجا وارد کنید" value="{{old('code',$discount->code)}}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <input type="text" class="form-control" name="percent" id="inputEmail3"
                               placeholder="درصد تخفیف را اینجا وارد کنید"
                               value="{{old('percent',$discount->percent)}}"></div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-sm-10">
                        <select name="users[]" id="users" class="form-control" multiple>
                            ...
                            @foreach(\App\Models\User::all() as $user)
                                <option
                                    value="{{$user->id}}"{{in_array($user->id,$discount->users->pluck('id')->toArray()) ? 'selected' :''}}>{{$user->name}}</option>
                ...
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label"> زمان مجاز استفاده از کد </label>

                    <div class="col-sm-10">
                        <input type="datetime-local" class="form-control" name="expired_at" id="inputEmail3"
                               placeholder="زمان مجاز کد تخفیف را اینجا وارد کنید" value="{{old('expired_at',\Carbon\Carbon::parse($discount->expired_at)->format('Y-m-d\TH:i:s'))}}">
                    </div>
                </div>
